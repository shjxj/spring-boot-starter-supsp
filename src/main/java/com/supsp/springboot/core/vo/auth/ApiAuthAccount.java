...
import com.supsp.springboot.core.vo.auth.api.ApiAccountPost;
import com.supsp.springboot.core.vo.auth.api.ApiAccountShop;
...
@SensitiveData
public class ApiAuthAccount extends BaseAuthAccount implements IAuthAccount {

    @Serial
    private static final long serialVersionUID = 2721671749738146855L;

    @Schema(title = "登录用户类型")
    protected AuthMemberType memberType = AuthMemberType.api;

    @Schema(title = "组织列表")
    private List<ApiAccountOrg> orgs;

    @Schema(title = "门店列表")
    private List<ApiAccountStore> stores;

    @Schema(title = "岗位列表")
    private List<ApiAccountPost> posts;

    @Schema(title = "组织信息")
    private ApiAccountOrg org;

    @Schema(title = "门店信息")
    private ApiAccountStore store;

    @Schema(title = "店铺信息")
    private ApiAccountShop shop;
    ...
    public <T extends BaseAccountOrg> void setOrgs(List<T> orgs) {
        try {
            this.orgs = (List<ApiAccountOrg>) orgs;
        } catch (Exception e) {
            // log
        }
    }
    ...
    public <T extends BaseAccountStore> void setStores(List<T> stores) {
        try {
            this.stores = (List<ApiAccountStore>) stores;
        } catch (Exception e) {
            // log
        }
    }
    ...
    public <T extends BaseAccountPost> void setPosts(List<T> posts) {
        try {
            this.posts = (List<ApiAccountPost>) posts;
        } catch (Exception e) {
            // log
        }
    }
    ...
    public <T extends BaseAccountStore> void setStore(T store) {
        try {
            this.store = (ApiAccountStore) store;
        } catch (Exception e) {
            // log
        }
    }
